...
class SeparableConvBnRelu(pl.LightningModule):
...
class GlobalAvgPool2d(pl.LightningModule):
    def __init__(self):
...
class BNRefine(pl.LightningModule):
...
class RefineResidual(pl.LightningModule):
    def __init__(self, in_planes, out_planes, ksize, has_bias=False,
                 has_relu=False, norm_layer=nn.BatchNorm2d, bn_eps=1e-5):
...
# For BiSeNet
class AttentionRefinement(pl.LightningModule):
...
class FeatureFusion(pl.LightningModule):
    def __init__(self, in_planes, out_planes,
